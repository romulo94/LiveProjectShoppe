...
NEW_URL=/config/mfe/util-state/$VERSION/shoppe-util-state.js
cat ./import-map.json | ./jq --arg NEW_URL "$NEW_URL" '.imports["@shoppe/util-state"] = $NEW_URL' > new.importmap.json
aws s3 cp dist s3://mfe-shoppe/config/mfe/util-state/$VERSION --recursive
