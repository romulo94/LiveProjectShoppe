...
NEW_URL=/config/mfe/app-account/$VERSION/shoppe-app-account.js
cat ./import-map.json | ./jq --arg NEW_URL "$NEW_URL" '.imports["@shoppe/app-account"] = $NEW_URL' > new.importmap.json
aws s3 cp dist s3://mfe-shoppe/config/mfe/app-account/$VERSION --recursive
